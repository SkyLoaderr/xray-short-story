...
void change_game_time(s32 days, s32 hours, s32 mins)
{
...
void change_level(Fvector3 position, Fvector3 direction, u32 level_vertex_id, GameGraph::_GRAPH_ID game_graph_id)
{
	NET_Packet				P;
...
bool is_level_present_and_ready()
{
...
CScriptGameObject* get_target_obj()
{
...
//-		def("map_add_object_spot_complex",		map_add_object_spot_complex),
		...
		def("present_and_ready",					is_level_present_and_ready),
		...
		def("get_target_object",					get_target_obj),
		def("get_target_distance",					get_target_dist),
