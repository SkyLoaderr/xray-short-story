...
void CActor::cam_Update(float dt, float fFOV)
{
	...
	const float HeightInterpolationSpeed = 8.f;

	if (!fsimilar(m_fCamCurrentHeight, CameraHeight()))
		m_fCamCurrentHeight = (m_fCamCurrentHeight * (1.0f - HeightInterpolationSpeed*dt)) + (CameraHeight() * HeightInterpolationSpeed*dt);
	
	Fvector point		= {0, m_fCamCurrentHeight + current_ik_cam_shift, 0};
