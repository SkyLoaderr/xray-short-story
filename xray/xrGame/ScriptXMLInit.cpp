...
int CScriptXmlInit::InitMMShniagaNewPage(LPCSTR path, CUIMMShniaga* parent)
{
	if (!parent)
		Debug.fatal(DEBUG_INFO, "Can't call InitMMShniagaNewPage with path '%s' because parent doen't exist", path);

	return parent->AddPage(m_xml, path);
}
...
void CScriptXmlInit::script_register(lua_State *L){
	...
	[
		...
		.def("InitMMShniagaNewPage",	&CScriptXmlInit::InitMMShniagaNewPage)
